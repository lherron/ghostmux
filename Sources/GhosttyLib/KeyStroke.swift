...
public func keyStrokeForScalar(_ scalar: UnicodeScalar) -> KeyStroke? {
  ...
  case 0x30...0x39:
    return KeyStroke(
      key: "digit\(scalar.value - 0x30)", mods: [], text: String(scalar),
  ...
  case "{":
    return KeyStroke(key: "bracketLeft", mods: ["shift"], text: "{", unshiftedCodepoint: 0x5B)
  case "]": return KeyStroke(key: "bracketRight", mods: [], text: "]", unshiftedCodepoint: 0x5D)
  case "}":
    return KeyStroke(key: "bracketRight", mods: ["shift"], text: "}", unshiftedCodepoint: 0x5D)
