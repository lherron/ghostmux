...
func printInfo() {
  ...
  let myName =
    myUUID != "(not in a Ghostty terminal)"
...
func printList(json: Bool) throws {
  ...
  for terminal in terminals {
    ...
    rows.append(
      Row(
        nameCol: nameCol, shortId: shortId, cwdShort: cwdShort, identitySuffix: suffix,
        focused: terminal.focused))
  }
...
func handleIdentity(args: [String]) throws {
  ...
  let flagMap = [
    "--role": "ghostchat.role", "--project": "ghostchat.project", "--task": "ghostchat.task",
  ]
    ...
    if let metaKey = flagMap[args[i]] {
      ...
      if value.contains(" ") || value.contains("\t") {
        throw GhosttyError.message(
          "identity fields must not contain spaces (got \"\(value)\", try \"\(value.replacingOccurrences(of: " ", with: "-"))\")"
        )
      }
